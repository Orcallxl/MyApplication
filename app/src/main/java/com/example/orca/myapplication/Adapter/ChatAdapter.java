package com.example.orca.myapplication.Adapter;
...
public class ChatAdapter extends ArrayAdapter<ChatEntity> {
    ...
    public ChatAdapter(Context context, int textViewResourceId, List<ChatEntity>
            objects) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if ( chatEntity.isComeMsg()==true ) {
// 如果是收到的消息，则显示左边的消息布局，将右边的消息布局隐藏
            viewHolder.leftLayout.setVisibility(View.VISIBLE);
            viewHolder.rightLayout.setVisibility(View.GONE);
            viewHolder.leftMsg.setText(chatEntity.getContent());
        } else if(chatEntity.isComeMsg()==false) {
// 如果是发出的消息，则显示右边的消息布局，将左边的消息布局隐藏
            viewHolder.rightLayout.setVisibility(View.VISIBLE);
            viewHolder.leftLayout.setVisibility(View.GONE);
            viewHolder.rightMsg.setText(chatEntity.getContent());
        }
        return view;
    }
    class ViewHolder {
